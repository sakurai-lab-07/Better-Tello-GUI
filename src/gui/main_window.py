...
class TelloApp:
    ...
    def _create_widgets(self):
        ...
        ttk.Label(timeline_frame, text="解析ログ", style="Header.TLabel").pack(
            anchor="w", padx=5
        )
    ...
    def add_drone_entry(self, name=None, ip=""):
        ...
        entry.insert(0, ip)
        telemetry_label = ttk.Label(
            row_frame, text="--- / ---", width=12, style="BatteryOffline.TLabel"
        )
        ...
        widget_dict.update(
            {
                ...
                "ip_widget": entry,
                "label_widget": label,
                "telemetry_widget": telemetry_label,
            }
        )
                ...
                for widget in self.drone_entry_widgets:
                    ...
                    if name in self.controllers:
                        ...
                        if state["active"]:
                            ...
                            if bat < 20:
                                label.config(text=text, style="BatteryLow.TLabel")
                            else:
                                label.config(text=text, style="BatteryOK.TLabel")
                        else:
    ...
    def auto_connect_wifi(self):
        if NetworkManager is None:
            messagebox.showerror(
                "エラー", "NetworkManagerモジュールが読み込めませんでした。"
            )
            return
        if not messagebox.askyesno("Wi-Fi自動接続", "Wi-Fi自動接続を開始しますか？"):
            return
        ...
        try:
            ...
            if connected_list:
                self.log(
                    {
                        "level": "SUCCESS",
                        "message": f"{len(connected_list)} 台接続。IP取得待ち...",
                    }
                )
    ...
    def auto_detect_drones(self):
        ...
        if self.drone_entry_widgets:
            if not messagebox.askyesno("確認", "リストを上書きしますか？"):
                return
        ...
        for i, t in enumerate(found):
            ...
            self.drone_entry_widgets[-1]["label_widget"].config(
                text=f"{name} ({t['ssid']}):"
            )
    ...
    def parse_scratch_project(self):
        ...
        if self.schedule:
            self.schedule_text.insert(
                tk.END, f"--- 解析完了 (総時間: {self.total_time:.2f}s) ---\n", "HEADER"
            )
            current_line = 2
        ...
        self.log_queue.put(
            {"level": "DEBUG", "message": f"Parsed schedule: {self.schedule}"}
        )  # 追加
    ...
    def connect_drones(self):
        ...
        for w in self.drone_entry_widgets:
            ip = w["ip_widget"].get()
            if ip:
                config.append({"name": w["name"], "pc_ip": ip})
        if not config:
            self.connect_btn["state"] = "normal"
            return
    ...
    def _connect_worker(self, config):
        runner = ShowRunner(
            config, self.schedule, self.stop_event, self.log_queue, self.total_time
        )
        runner.connect()
    ...
    def start_show(self):
        ...
        if self.controllers:
            for name, ctrl in self.controllers.items():
                st = ctrl.get_state()
                if st["active"] and st["bat"] < 20:
                    low_battery.append(f"{name} ({st['bat']}%)")
        if low_battery:
            messagebox.showerror(
                "離陸ロック", "バッテリー不足:\n" + "\n".join(low_battery)
            )
        ...
        if hasattr(runner, "set_music_player"):
            runner.set_music_player(self.music_player)
        else:
            runner.music_player = self.music_player
