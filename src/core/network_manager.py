...
class NetworkManager:
            ...
            if ssid and ssid.upper().startswith("TELLO-"):
                ...
                if ip:
                    tello_connections.append(
                        {"interface": iface["name"], "ssid": ssid, "ip": ip}
                    )
    ...
    def connect_all_tellos(self, log_callback=None):
        ...
        if not self.is_windows:
            if log_callback:
                log_callback("エラー: Windows専用機能です。")
            return []
        ...
        # 2. スキャン (リトライ機能付き)
        if log_callback:
            log_callback("周囲のネットワークをスキャン中... (最大10秒)")
        found_tellos = self._scan_tello_networks_robust()

        if not found_tellos:
            if log_callback:
                log_callback(
                    "TELLOネットワークが見つかりませんでした。\n・Telloの電源が入って黄色点滅していますか？"
                )
            return []

        if log_callback:
            log_callback(f"検出: {len(found_tellos)}機 ({', '.join(found_tellos)})")
        ...
        for i in range(count):
            ...
            if log_callback:
                log_callback(f"接続試行: {iface_name} -> {target_ssid}")

            if self._connect_interface_to_ssid(iface_name, target_ssid):
                connected_ssids.append(target_ssid)
                if log_callback:
                    log_callback(f"コマンド送信成功: {iface_name} -> {target_ssid}")
            else:
                if log_callback:
                    log_callback(f"コマンド送信失敗: {iface_name} -> {target_ssid}")

        # 4. 待機
        if connected_ssids:
            if log_callback:
                log_callback("接続の確立を待機しています... (IP取得待ち)")
            for i in range(15):
                time.sleep(1)
                ...
                if len(current) >= len(connected_ssids):
                    if log_callback:
                        log_callback(
                            f"接続完了確認: {len(current)}台がネットワークに参加しました。"
                        )
                    break
    ...
    def _connect_interface_to_ssid(self, interface_name, ssid):
        ...
        try:
            ...
            connect_cmd = (
                f'netsh wlan connect name="{ssid}" interface="{interface_name}"'
            )
            subprocess.run(
                connect_cmd,
                shell=True,
