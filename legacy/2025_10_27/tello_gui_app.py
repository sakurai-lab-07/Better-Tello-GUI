...
class TelloApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Tello Scratch ドローンショー・コントローラー (最終安定版)")
    ...
    def setup_styles(self):
        ...
        s.theme_use("clam")
        s.configure(
            ".", background="#f0f0f0", foreground="black", font=self.font_normal
        )
        ...
        s.map(
            "Accent.TButton",
            background=[("active", "#005f9e"), ("disabled", "#5a9fd4")],
        )
        s.configure(
            "Stop.TButton",
            font=self.font_normal,
        ...
        s.map(
            "Stop.TButton", background=[("active", "#a4262c"), ("disabled", "#e89c9f")]
        )
    ...
    def _create_widgets(self):
        ...
        file_frame = ttk.LabelFrame(
            left_frame, text="② プロジェクト選択 & 解析", padding="10"
        )
        ...
        audio_frame = ttk.LabelFrame(
            left_frame, text="④ 音源ファイル (オプション)", padding="10"
        )
        ...
        ttk.Button(
            audio_frame, text="🎵 音楽ファイルを選択...", command=self.select_audio_file
        ).pack(fill="x")
    ...
    def load_config(self):
        try:
            with open("tello_config.json", "r") as f:
            ...
            self.log(
                {
                    "level": "INFO",
                    "message": f"tello_config.json から設定を読み込みました。",
                }
            )
    ...
    def save_config(self):
        ...
        try:
            with open("tello_config.json", "w") as f:
                json.dump(config_data, f, indent=4)
            self.log(
                {
                    "level": "INFO",
                    "message": f"tello_config.json に設定を保存しました。",
                }
            )
        ...
        if self.schedule or parser.has_any_valid_action:
            ...
            for time, events in grouped_events.items():
                ...
                for event in events:
                    ...
                    log_msg = ""
                    if evt_type == "TAKEOFF":
                        log_msg = f"{time: >6.2f}s | {event.get('target', 'N/A'): <8} | {event.get('text', '')}\n"
                    ...
                    self.schedule_text.insert(tk.END, log_msg, evt_type or "INFO")
                    current_line += 1
    ...
    def _reset_ui_to_file_selected_state(self):
        ...
        self.stop_btn["state"] = "disabled"
        self.connect_btn.config(text="📡 ドローンに接続")
        self.show_status.set(f"ファイル選択済み。解析してください。")
        ...
        try:
            while not self.log_queue.empty():
                ...
                if isinstance(log_item, dict):
                    level, message = log_item.get("level", "INFO"), log_item.get(
                        "message", ""
                    )
                else:
                    level, message = "INFO", str(log_item)
